...
#include "BeJwrMco0rMg0K0otLs0pAQAgzAT3.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<int32 x,int32 result>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("x");
        TypeOffset t(MY_OFFSETOF(x_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "x") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, x_);
   else
     istr >> x_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, x_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   if (c != ',') { istr.setstate(std::ios_base::failbit); return; }
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "x=", get_x()) << ",";
