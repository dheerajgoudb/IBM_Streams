...
#ifndef BEJWRMS02YUVMTFUYTKXPLTYTYSZJSS02S0_1NS0KTCJNNYS8RLQYA5ZCNAM_H_
#define BEJWRMS02YUVMTFUYTKXPLTYTYSZJSS02S0_1NS0KTCJNNYS8RLQYA5ZCNAM_H_
...
public:
    ...
    typedef SPL::float64 bonus_type;

    enum { num_attributes = 5 } ;
    
    SELF() : Tuple(), name_(), age_(), title_(), gender_(), bonus_() {}
    SELF(const Self & ot) : Tuple(), name_(ot.name_), age_(ot.age_), title_(ot.title_), gender_(ot.gender_), bonus_(ot.bonus_) 
      { constructPayload(ot); }
    SELF(const name_type & _name, const age_type & _age, const title_type & _title, const gender_type & _gender, const bonus_type & _bonus) : Tuple(), name_(_name), age_(_age), title_(_title), gender_(_gender), bonus_(_bonus) { }
    ...
    bonus_type & get_bonus() { return bonus_; }
    const bonus_type & get_bonus() const { return bonus_; }
    void set_bonus(const bonus_type & _bonus) { bonus_ = _bonus; }
    ...
    {
        buf << name_ << age_ << title_ << gender_ << bonus_;
    }
    ...
    size_t hashCode() const
    {
        ...
        s = 37 * s + std::tr1::hash<bonus_type >()(bonus_);
        return s;
    }
    
    size_t getSerializedSize() const
    {
        size_t size = sizeof(SPL::uint8)+sizeof(SPL::float64);
...
#undef SELF
#endif // BEJWRMS02YUVMTFUYTKXPLTYTYSZJSS02S0_1NS0KTCJNNYS8RLQYA5ZCNAM_H_ 
